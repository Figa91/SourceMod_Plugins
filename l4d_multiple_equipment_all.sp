...
new bool:CountUse[MAXPLAYERS+1];
...
public Plugin:myinfo = 
{
	...
	author = "Pan Xiaohai & Marcus101RR &fix Figa",
...
public OnPluginStart()
{
	...
	l4d_me_slot[2]=CreateConVar("l4d_me_slot2", "0", "0:diable slot 2 (pipebomb ), 1,enable");
	l4d_me_slot[3]=CreateConVar("l4d_me_slot3", "0", "0:diable slot 3 (medkits ), 1,enable");
	l4d_me_slot[4]=CreateConVar("l4d_me_slot4", "0", "0:diable slot 4 (painpills), 1,enable");
	...
	LoadTranslations("MEquipments.phrases");
}
...
bool g_bFinale()
{
	...
	if (StrEqual(mapname, "l4d_farm05_cornfield")			|| 
		...
		StrEqual(mapname, "l4d_river03_port")				||
		...
		StrEqual(mapname, "GoingUp")						||
		StrEqual(mapname, "l4d_derailed_finale3ver")		||
		StrEqual(mapname, "l4d_149_5")						||
		StrEqual(mapname, "l4d_ruinas")						||
		StrEqual(mapname, "c3m4_plantation")				||
		StrEqual(mapname, "Dach")							||
		StrEqual(mapname, "Domki")							||
		StrEqual(mapname, "rombu05")						||
		StrEqual(mapname, "l4d_nt05_wake")					||
		StrEqual(mapname, "l4d_sh11_nowhere")				||
		StrEqual(mapname, "mnac")) return true;
	return false;
}
...
public bot_player_replace(Handle:Spawn_Event, const String:Spawn_Name[], bool:Spawn_Broadcast)
{
	...
	CreateTimer(1.0, Timer_EnableClient, client);
}
...
public Action:round_start(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	for(new i=1; i<=MaxClients; i++)
	{
		if(IsClientConnected(i) && !IsFakeClient(i)) CreateTimer(1.0, Timer_EnableClient, i);
	}
}
...
public Action:Timer_EnableClient(Handle:timer, any:client)
{
	if(MeEnable[client]==false)
	{
		EnableClient(client,true);
		ModeSelectMenu(client);
		//PrintToChat(client, "[Equipment] MeEnable!");
	}
	if(FirstRun[client]==true)
	{
		...
		AttachAllEquipment(client);
		//PrintToChat(client, "[Equipment] FirstRun!");
	}
}
...
public Action:Timer_AnonceHave(Handle:timer, any:client)
{
	CountUse[client] = false;
}
public Action:OnWeaponCanUse(client, weapon)
{
	if(!IsFakeClient(client))
	{
		...
		if(StrContains(iWeaponName, "weapon_hunting_rifle", false) != -1)
		{
			if(StrEqual(ItemName[client][0], "weapon_hunting_rifle"))
			{
				if (!CountUse[client])
				{
					PrintToChat(client, "%t", "HaveHuntingRifle");
					CountUse[client] = true;
					CreateTimer(5.0, Timer_AnonceHave, client);
				}
				return Plugin_Handled;
			}
		}
		else if(StrContains(iWeaponName, "weapon_rifle", false) != -1)
		{
			if(StrEqual(ItemName[client][0], "weapon_rifle"))
			{
				if (!CountUse[client])
				{
					PrintToChat(client, "%t", "HaveRifle");
					CountUse[client] = true;
					CreateTimer(5.0, Timer_AnonceHave, client);
				}
				return Plugin_Handled;
		...
		else if(StrContains(iWeaponName, "weapon_smg", false) != -1)
		{
			if(StrEqual(ItemName[client][0], "weapon_smg"))
			{
				if (!CountUse[client])
				{
					PrintToChat(client, "%t", "HaveSmg");
					CountUse[client] = true;
					CreateTimer(5.0, Timer_AnonceHave, client);
				}
				return Plugin_Handled;
		...
		else if(StrContains(iWeaponName, "weapon_pumpshotgun", false) != -1)
		{
			if(StrEqual(ItemName[client][0], "weapon_pumpshotgun"))
			{
				if (!CountUse[client])
				{
					PrintToChat(client, "%t", "HavePumpshotgun");
					CountUse[client] = true;
					CreateTimer(5.0, Timer_AnonceHave, client);
				}
				return Plugin_Handled;
		...
		else if(StrContains(iWeaponName, "weapon_autoshotgun", false) != -1)
		{
			if(StrEqual(ItemName[client][0], "weapon_autoshotgun"))
			{
				if (!CountUse[client])
				{
					PrintToChat(client, "%t", "HaveAutoshotgun");
					CountUse[client] = true;
					CreateTimer(5.0, Timer_AnonceHave, client);
				}
				return Plugin_Handled;
