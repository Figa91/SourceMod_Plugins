 ...
 *   sm_hpregeneration_minhealth - Min health to regenerate to, based on the control mode.
 *     - 1 = Regenerate to 1 health (min. default)
 *     - 50 = Regenerate to 50 health
...
public event_PlayerHurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	if (isHooked)
	{
		...
		if (!client || IsFakeClient(client) && !GetConVarInt(cvarBot))
			return;			
